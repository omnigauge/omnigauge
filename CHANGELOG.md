...
- No em-dashes or en-dashes anywhere a reader meets OmniGauge: 321 of them swept out of the CLI's
  panels, docstrings and notifications, the site and llms.txt, the READMEs (PyPI's included), the
  providers, the changelog and the rest of the repository, to plain dashes or commas.
  tests/test_dashes.py scans the whole tracked tree and fails on the next one; the tests' own
  banned-glyph lists, the site harness's regex and base64 font lines are the only sanctioned
  occurrences. No behaviour change; the version moves because rendered strings did.
